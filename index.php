...
class SimpleGallery {
    ...
    /**
     * Returns the data of the current and valid user.
     * 
     * @return array The user data or NULL if not available.
     */
    public function getCurrentUser() {
        $userAndPwd = $this->getUserAndPassword();
        if (is_array($userAndPwd)) {
            foreach ($this->getUsers() as $user) {
                if (trim(strtolower($user['name'])) == $userAndPwd['user']) {
                    return $user;
                }
            }
        }
    ...
    /**
     * Gets the mode value that was submitted by the requesting client.
     * 
     * @return string The mode value from the client.
     */
    ...
    /**
     * Returns the username and password that were submitted by the
     * requesting client via BasicAuth.
     * 
     * @return array The username and password or NULL if no data is
     *               available.
     */
    public function getUserAndPassword() {
        $headers = getallheaders();
        foreach (getallheaders() as $headerName => $headerVal) {
            if ('authorization' == trim(strtolower($headerName))) {
                $headerVal = trim($headerVal);
                if (false !== stripos($headerVal, 'basic ')) {
                    $usrAndPwd = base64_decode(trim(substr($headerVal, 6)));
                    if (false !== $usrAndPwd) {
                        $pwd = null;
        
                        $sepPos = stripos($usrAndPwd, ':');
                        if (false !== $sepPos) {
                            $user = substr($usrAndPwd, 0, $sepPos);
                            $pwd  = substr($usrAndPwd, $sepPos + 1);
                        }
                        else {
                            $user = $usrAndPwd;
                        }
    ...
    /**
     * Checks if the current client is authorized.
     * 
...
foreach ($GLOBALS['conf']['files'] as $confFile) {
    ...
    if (false === $customConf) {
        die(sprintf("Could not load config file '%s'!",
                    $customConfigFile));
    }
...
if ($sg->isBlocked()) {
    // blocked
    $executeGallery = false;
}
else if (!$sg->isAuthorized()) {
    // not authorized
    $executeGallery = false;
    
    header(sprintf('WWW-Authenticate: Basic realm="SimpleGallery"'));
}
